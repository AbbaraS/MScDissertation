...
def slice(case):
    ...
    for axis_label, axis_idx, nz in axis_info:
        ...
        if len(idxs) == 0:
            ...
            log(f"[{axis_label}] LV empty on this axis; using fallback indices: {idxs}")
        ...
        log(f"axis: {axis_label}, indices: {idxs}", True)
        for i in idxs:
            log(f"  slice {axis_label}={i}", True)
...
def save_slices(case, results):
    for axis, data in results.items():
        ...
        for i, (ct_slice, mask_slice) in enumerate(zip(ct_slices, mask_slices)):
            print(f"Saving CT & mask slice {axis}_{i}")
