...
from monai.transforms import (
	...
	CropForegroundd, NormalizeIntensityd, Resized, EnsureTyped, RandAdjustContrastd,
...
class DataLoaderFactory:
	...
	def create_outer_loadersSingleView(self, outer_fold_id):
		...
		outer_fold_struct = all_folds_data[outer_fold_id]
		fold_pool = [main[i] for i in outer_fold_struct['OUTER_FOLD_TRAIN_idx']]
		fold_pool_labels = [main[i]['label'] for i in outer_fold_struct['OUTER_FOLD_TRAIN_idx']]
		...
		test_fold_data = [main[i] for i in outer_fold_struct['OUTER_FOLD_TEST_idx']]
	...
	def create_holdout_loader(self, outer_fold_id):
		"""
		Generates train and test dataloaders for a specific outer fold.
		"""
		holdout = [i for i in self.main_dataset if i["pool"] == 'holdout']
		...
		test_transforms = get_val_test_transforms(outer_stats)
		test_dataset = CardiacCTDataset(holdout, test_transforms, outer_stats)
		test_loader = DataLoader(test_dataset, batch_size=8, shuffle=False, num_workers=self.num_workers)
...
def get_train_transforms(stats):   #REMEMBER: try different augmentations
	...
	] # RandZoomd,RandSpatialCropd,RandCoarseDropoutd
...
class CardiacCTDataset(Dataset):
	...
	def __getitem__(self, idx):
		...
		# MONAI standard orientation (LAS) is (H, W, D) which corresponds to
