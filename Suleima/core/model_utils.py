...
def train_INNER_MLP(model, train_loader, val_loader, experiment):
	...
	epochs = hypers['Epochs']
	optimizer = optim.Adam(model.parameters(), lr= hypers['LR'], weight_decay=hypers['WD'])
	scheduler = ReduceLROnPlateau(optimizer, mode='min', patience=2, factor=0.99,  threshold=0.5, min_lr=8e-4)
	criterion = nn.BCEWithLogitsLoss()
	best_V_loss = float('inf')
	P_counter = 0
	val_N=len(val_loader.dataset)
	...
	for epoch in range(epochs):
		...
		if V_loss < best_V_loss:
			...
			P_counter += 1
		log.info(f"	 MLP;	{experiment['ExpID']}; OUT:	{experiment['OUTER_FOLD']}; IN:	{experiment['INNER_FOLD']}; HPset: {hypers['HPset']};	E: {epoch};   	{T_loss}; 	{V_loss};  	{P_counter}; 	{optimizer.param_groups[0]['lr']}")
		if P_counter >=3:
			return best_V_loss
	return best_V_loss
...
def train_INNER_RESNET(train_loader, val_loader, experiment):
	...
	P_counter = 0
	scheduler = ReduceLROnPlateau(optimizer, mode='min', patience=3, factor=0.9, threshold=0.01, min_lr=8e-4)
	criterion = nn.BCEWithLogitsLoss()
	best_V_loss = float('inf')
	val_N=len(val_loader.dataset)
	train_N=len(train_loader.dataset)
	...
	for epoch in range(epochs):
		...
		if V_loss < best_V_loss:
			...
			P_counter += 1
		log.info(f"	 RESNET;	{experiment['ExpID']}; OUT: {experiment['OUTER_FOLD']}; IN: {experiment['INNER_FOLD']}; 	HPset: {hypers['HPset']};  	E: {epoch}; 	{T_loss}; 		{V_loss}; 	 	{P_counter}; 	{optimizer.param_groups[0]['lr']}")
		if P_counter >= hypers['P']:
			return best_V_loss
	return best_V_loss
